...
(function(){
  if(localStorage.getItem('targetDate') != null && localStorage.getItem('eventNameText') != null){
...
function btnEvent(){
  if(eventName.value != '' && dayInput.value != '' && yearInput.value.length === 4){
    if(parseInt(minuteInput.value) >= 0 && parseInt(minuteInput.value) <= 59 && parseInt(hourInput.value) >= 0 && parseInt(hourInput.value) <= 23 && parseInt(dayInput.value) >= 1 && parseInt(dayInput.value) <= 31 && yearInput.value <= date.getFullYear() + 2){
      targetDate = new Date(`${monthInput.options[monthInput.selectedIndex].text} ${dayInput.value}, ${yearInput.value} ${hourInput.value !== '' ? hourInput.value : 0}:${minuteInput.value !== '' ? minuteInput.value : 0}:00`);
...
function updateTime(){
  ...
  minute.textContent = parseInt(diff % 60).toString().length == 1 ? `0${parseInt(diff % 60)}` : parseInt(diff % 60);
  diff = parseInt(diff / 60);
  hour.textContent = parseInt(diff % 24).toString().length == 1 ? `0${parseInt(diff % 24)}` : parseInt(diff % 24);
