...
form.addEventListener('submit', (e) => {
  ...
  if(username.value != '' && email.value != '' && password.value != ''){
    if(username.value.length > 1 && password.value.length > 7){
      if(reUsername.exec(username.value) === null && reEmail.test(email.value) === true && rePassword.exec(password.value) === null){
        username.style.boxShadow = '0 0 0 2px #0f0';
        email.style.boxShadow = '0 0 0 2px #0f0';
        password.style.boxShadow = '0 0 0 2px #0f0';
        
        console.log(reUsername.exec(username.value), reEmail.test(email.value), rePassword.exec(password.value));
      } else {
        console.log(reUsername.exec(username.value), reEmail.test(email.value), rePassword.exec(password.value));
